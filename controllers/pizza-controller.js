...
const pizzaController = {
  // get all pizzas
  // use select option inside & outside of populate(), so Mongoose will ignore __v fields
  // .sort({ _id: -1 }) to sort in DESC order by the _id value
  getAllPizza(req, res) {
    Pizza.find({})
      .populate({
        path: "comments",
  ...
  updatePizza({ params, body }, res) {
    ...
    Pizza.findOneAndUpdate({ _id: params.id }, body, {
      new: true,
      runValidators: true,
    }) // { new: true }, it will return the original document
      .then((dbPizzaData) => {
        if (!dbPizzaData) {
          res.status(404).json({ message: "No pizza found with this id!" });
          return;
